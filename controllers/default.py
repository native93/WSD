...
def form_file(name,word,hindi=None,typ,property=None,relations=None):
	f=open(request.folder+'uploads/'+name,'w')
	f.write("(defrule "+name+"\n(id­root ?id "+word+")\n?mng <­(meaning_to_be_decided ?id)\n(id­cat_coarse ?id "+typ+")(not (kriyA­object ?id ?obj))\n=>\n(retract ?mng)(assert (id­wsd_root_mng ?id baDa))\n(if ?*debug_flag* then\n(printout wsd_fp \"(dir_name­file_name­rule_name­id­wsd_root_mng  \" ?*wsd_dir* \"\ngrow.clp  grow0  \" ?id \" baDa )\" crlf))\n)")
	f.close()
@auth.requires_login()
def home():
    t = auth.user.id
